...
/**
 * Idempotency test for UserRegisteredConsumer (NOTIF-01).
 *
...
@ActiveProfiles("test")
@Testcontainers
class UserRegisteredConsumerIdempotencyTest {
    ...
    void duplicateEvent_writesNotificationExactlyOnce_processedEventsCountIs1() throws Exception {
        ...
        String envelopeJson = """
            {"eventId":"%s","eventType":"user.registered","eventVersion":1,
             "occurredAt":"%s","correlationId":"%s","causationId":null,
             "producer":"identity-service",
             "payload":{"userId":"%s","email":"alice@buyer.example.com",
               "fullName":"Ali Kendir","registeredAt":"2026-04-30T12:00:00Z"}}
            """.formatted(eventId, Instant.now(), UUID.randomUUID(), userId);
        ...
        consumer.handleUserRegistered(m);
        consumer.handleUserRegistered(m);
